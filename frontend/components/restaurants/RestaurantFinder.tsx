...
function StarRating({ rating }: { rating: string }) {
  ...
  return (
    <div className="flex items-center gap-0.5">
      ...
      <span className="text-xs text-[var(--fg-muted)] ml-1">{num}</span>
    </div>
  );
}
...
const TIER_COLORS: Record<string, string> = {
  ...
  "fine dining": "bg-violet-500/20 text-emerald-300 border-violet-500/30",
};
...
export function RestaurantFinder() {
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-8">
      <div className="mb-8">
        <h1 className="font-display text-4xl md:text-5xl leading-[1] tracking-tight text-[var(--fg)] mb-2">Restaurant Finder</h1>
        <p className="text-[var(--fg-muted)]">Top eats with price ranges · Must-try dishes · Web-sourced</p>
      </div>
      ...
      <div className="glass rounded-2xl p-5 mb-6">
        <div className="grid sm:grid-cols-3 gap-3 mb-4">
          ...
          <div>
            <label className="text-xs font-medium text-[var(--fg-muted)] mb-1 block">Cuisine</label>
...
function RestaurantCard({ restaurant: r }: { restaurant: Restaurant }) {
  ...
  return (
    ...
    >
      ...
      {mustTry.length > 0 && (
        <div>
          <p className="text-xs text-[var(--fg-muted)] mb-1">Must try:</p>
