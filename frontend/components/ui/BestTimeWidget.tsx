    ...
    <div className="glass rounded-2xl p-5">
      ...
      <div className="flex gap-2 mb-4">
        ...
        <button onClick={go} disabled={isFetching || !dest.trim()}
          className="flex items-center gap-1.5 bg-indigo-600 hover:bg-indigo-500 text-white px-4 py-2 rounded-lg text-sm font-medium transition-colors disabled:opacity-60">
      ...
      <AnimatePresence mode="wait">
        {data && data.months?.length > 0 && (
          <motion.div key={data.destination} variants={staggerContainer} initial="hidden" animate="show">
            <motion.div variants={fadeUp} className="grid grid-cols-6 gap-1.5 mb-3">
              {data.months.map((m) => (
                <div key={m.month} className="text-center group relative">
                  ...
                  <span className="text-[9px] text-[#8892b0] mt-1 block">{m.month.slice(0, 3)}</span>
                  <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-1 hidden group-hover:block z-10 w-40 glass rounded-lg p-2 text-[10px] text-left pointer-events-none">
                    ...
                    <p className="text-[#8892b0]">{m.weather}</p>
                    <p className="text-[#8892b0]">Crowds: {m.crowds}</p>
                  </div>
                </div>
              ))}
            </motion.div>
            ...
            <motion.p variants={fadeUp} className="text-xs text-[#8892b0] leading-relaxed">{data.summary}</motion.p>
          </motion.div>
        )}
      </AnimatePresence>
    </div>
