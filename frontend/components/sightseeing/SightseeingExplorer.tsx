...
const CATEGORY_COLORS: Record<string, string> = {
  ...
  culture:     "bg-violet-500/20 text-emerald-300 border-violet-500/30",
  beach:       "bg-sky-500/20 text-sky-300 border-sky-500/30",
  museum:      "bg-emerald-500/20 text-emerald-300 border-emerald-500/30",
...
export function SightseeingExplorer() {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8">
      <div className="mb-8">
        <h1 className="font-display text-4xl md:text-5xl leading-[1] tracking-tight text-[var(--fg)] mb-2">Sightseeing Explorer</h1>
        <p className="text-[var(--fg-muted)]">Top attractions with entry fees · Nearby day trips ≤ 2 hours · Web-sourced data</p>
      </div>
      ...
      <div className="glass rounded-2xl p-4 mb-6 flex gap-3 flex-wrap">
        ...
        <button
          ...
          className="flex items-center gap-2 bg-emerald-600 hover:bg-emerald-500 text-white px-5 py-2 rounded-xl font-medium text-sm disabled:opacity-60 transition-colors">
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-8">
            ...
            <div className="flex items-center justify-between flex-wrap gap-3">
              <div>
                ...
                <p className="text-[var(--fg-muted)] text-sm">{result.attractions.length} attractions · {result.nearby_places.length} nearby places</p>
              </div>
              ...
              <div className="flex flex-wrap gap-2">
                <button
                  ...
                  className={cn("px-3 py-1 rounded-full text-xs font-medium border transition-colors",
                    filter === "all" ? "bg-emerald-600 text-white border-emerald-500" : "border-white/10 text-[var(--fg-muted)] hover:text-white"
                  )}>
                ...
                {usedCategories.map(cat => (
                  <button key={cat} onClick={() => setFilter(cat)}
                    className={cn(
                      ...
                      filter === cat
                        ? CATEGORY_COLORS[cat] ?? "bg-emerald-600 text-white border-emerald-500"
            ...
            {result.nearby_places.length > 0 && (
              <div>
                <div className="flex items-center gap-2 mb-4">
                  ...
                  <span className="text-xs text-[var(--fg-muted)] border border-white/10 rounded-full px-2 py-0.5">
                    ≤ 2 hours away
                  </span>
                </div>
        ...
        {!result && !mutation.isPending && (
          ...
          >
            <svg width="72" height="72" viewBox="0 0 72 72" fill="none" xmlns="http://www.w3.org/2000/svg">
              <circle cx="36" cy="36" r="34" fill="#17171b" stroke="#1f1f24" strokeWidth="1.5"/>
              <path d="M36 18 C36 18 22 30 22 40 C22 47.7 28.3 54 36 54 C43.7 54 50 47.7 50 40 C50 30 36 18 36 18Z" fill="#111114" stroke="#10b981" strokeWidth="1.5"/>
              <circle cx="36" cy="40" r="5" fill="#6366f1" opacity="0.8"/>
              <line x1="36" y1="18" x2="36" y2="14" stroke="#10b981" strokeWidth="1.5" strokeLinecap="round"/>
              <circle cx="36" cy="13" r="2" fill="#10b981"/>
            </svg>
            <div>
...
function AttractionCard({ attraction: a }: { attraction: Attraction }) {
  ...
  return (
    <motion.div
      ...
      className="glass rounded-xl p-4 hover:border-emerald-500/30 transition-all hover:-translate-y-0.5 flex flex-col gap-3"
    >
      ...
      <p className="text-[var(--fg-muted)] text-xs leading-relaxed flex-1">{a.description}</p>
      ...
      <div className="space-y-1.5">
        ...
        <div className="flex items-center gap-1.5 text-xs">
          ...
          {a.entry_cost_usd !== null && a.entry_cost_usd !== undefined && a.entry_cost_usd > 0 && (
            <span className="text-[var(--fg-muted)]">(~${a.entry_cost_usd})</span>
          )}
        </div>
        {a.time_needed && (
          <div className="flex items-center gap-1.5 text-xs text-[var(--fg-muted)]">
      ...
      {a.tips && (
        <div className="flex items-start gap-1.5 bg-emerald-500/10 border border-emerald-500/20 rounded-lg p-2 text-xs text-emerald-300">
    ...
    >
      <div className="flex items-start justify-between gap-2 mb-2">
        ...
        <div className="flex flex-col items-end gap-1 shrink-0">
          ...
          <span className="text-[10px] text-[var(--fg-muted)]">{p.distance_km}</span>
        </div>
      </div>

      <p className="text-[var(--fg-muted)] text-xs leading-relaxed mb-3">{p.highlights}</p>
      ...
      <div className="flex items-center justify-between text-xs">
        ...
        {p.how_to_get && p.how_to_get !== "null" && (
          <span className="text-[var(--fg-muted)]">{p.how_to_get}</span>
        )}
      </div>
    </motion.div>
  );
