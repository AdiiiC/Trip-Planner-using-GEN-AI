...
export function HotelFinder() {
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-8">
      <div className="mb-8">
        <h1 className="font-display text-4xl md:text-5xl leading-[1] tracking-tight text-[var(--fg)] mb-2">Hotel Finder</h1>
        <p className="text-[var(--fg-muted)]">Price suggestions · Booking.com · MakeMyTrip · Agoda</p>
      </div>

      <div className="glass rounded-2xl p-5 mb-6 space-y-4">
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-3">
          <div className="lg:col-span-1">
            <label className="text-xs font-medium text-[var(--fg-muted)] mb-1 block">City</label>
            <input value={city} onChange={e => setCity(e.target.value)}
              placeholder="Da Nang, Kuala Lumpur…" className="input-dark" />
          </div>
          <div>
            <label className="text-xs font-medium text-[var(--fg-muted)] mb-1 block">Check-in</label>
            <input type="date" value={checkIn} min={today}
              onChange={e => setCheckIn(e.target.value)} className="input-dark" />
          </div>
          <div>
            <label className="text-xs font-medium text-[var(--fg-muted)] mb-1 block">Check-out</label>
            <input type="date" value={checkOut} min={checkIn}
              onChange={e => setCheckOut(e.target.value)} className="input-dark" />
          </div>
          <div>
            <label className="text-xs font-medium text-[var(--fg-muted)] mb-1 block">Guests</label>
            <input type="number" min={1} max={10} value={guests}
              onChange={e => setGuests(+e.target.value)} className="input-dark" />
          </div>
          <div>
            <label className="text-xs font-medium text-[var(--fg-muted)] mb-1 block">Rooms</label>
            <input type="number" min={1} max={5} value={rooms}
              onChange={e => setRooms(+e.target.value)} className="input-dark" />
          </div>
          <div>
            <label className="text-xs font-medium text-[var(--fg-muted)] mb-1 block">Budget tier</label>
        ...
        <button onClick={() => mutation.mutate()}
          ...
          className="flex items-center gap-2 bg-emerald-600 hover:bg-emerald-500 text-white px-5 py-2.5 rounded-xl font-medium transition-colors disabled:opacity-60">
            ...
            <div className="flex items-center justify-between flex-wrap gap-3">
              ...
              {result.cheapest_per_night_inr > 0 && (
                <div className="glass rounded-xl px-4 py-2 border border-emerald-500/30">
                  <p className="text-xs text-[var(--fg-muted)]">From / night</p>
    ...
    >
      <div className="flex items-start justify-between gap-2 mb-2">
        <div>
          ...
          <div className="flex items-center gap-1 mt-0.5">
            ...
            <Star key={i} className={cn("w-3 h-3", i < (h.stars || 0) ? "fill-amber-400 text-amber-400" : "text-[var(--fg-muted)]")} />
          ))}
            {h.area && h.area !== "null" && (
              <span className="text-xs text-[var(--fg-muted)] ml-1">{h.area}</span>
            )}
          </div>
        </div>
        {isFirst && (
      ...
      <div className="flex items-end justify-between mt-3">
        <div>
          ...
          <p className="text-xs text-[var(--fg-muted)]">per night</p>
          {nights > 1 && h.total_inr > 0 && (
            <p className="text-xs text-[var(--fg-muted)]">Total: {formatINR(h.total_inr)}</p>
          )}
        </div>
        <div className="text-right">
          {h.rating && <p className="text-sm text-white font-medium">{h.rating}</p>}
          <p className="text-xs text-[var(--fg-muted)]">{h.source || "Booking.com"}</p>
          {h.url && (
            <a href={h.url} target="_blank" rel="noopener noreferrer"
              className="text-xs text-emerald-400 hover:text-emerald-300 flex items-center gap-0.5 justify-end mt-1">
      ...
      {h.highlights?.length > 0 && (
        <div className="flex flex-wrap gap-1 mt-3">
          {h.highlights.map((t, i) => (
            <span key={i} className="text-[10px] bg-white/5 border border-white/10 rounded-full px-2 py-0.5 text-[var(--fg-muted)]">
              {t}
            </span>
          ))}
        </div>
